...
{
public:
	EventFunction( asIScriptFunction* func )
	{
		function = func;
		object = nullptr;
		objectId = 0;
	...
	EventFunction( asIScriptFunction* func, void* obj, int tId ) // для делегации
	{
		function = func;
...
typedef map<string,_EventTree*>					  		EventBrunch;
...
{
	if( EventContext.size() <= ++CurrentEvent )
		EventContext.push_back( ASEngine->CreateContext( ) );
	return EventContext[CurrentEvent];
}
...
private:
	...
	bool run( ScriptString& script_eventName, ScriptHandle* handle, asIScriptContext* eventContext )
	{
		for( ASFunctionVecIt it = eventsbool.begin( ), end = eventsbool.end( ); it != end; it++ )
		{
			eventContext->Prepare( (*it)->function );
			if( (*it)->object )
				eventContext->SetObject( (*it)->object );
			eventContext->SetArgAddress( 0, &script_eventName );
			eventContext->SetArgObject( 1, handle );
			if( eventContext->Execute() == asEXECUTION_FINISHED && eventContext->GetReturnByte() == 0 )
				return false;
		}
...
public:
	...
	{
		if( !run( script_eventName, handle, eventContext ) )
			return false;
	...
	void AddCallback( ScriptString& script_eventName, EventFunction* func, string& currentName  )
	{
		if( currentName == "" )
		{
			if( func->function->GetReturnTypeId() == asTYPEID_BOOL )
				eventsbool.push_back( func );
			else if( func->function->GetReturnTypeId() == asTYPEID_VOID )
				eventsresult.push_back( func );
		}
		else
		{
			string section = findsection( currentName );
			...
			if( iterbrunch == brunchs.end( ) )
			{
				_EventTree* child = new _EventTree;
				child->parent = this;
				child->name = section;
				brunchs[section] = child;
				iterbrunch = brunchs.find( section );
			}
			iterbrunch->second->AddCallback( script_eventName, func, currentName );
		}
	}
	
	uint EraseCallback( ScriptString& script_eventName, EventFunction* func, string& currentName )
	{
		if( currentName == "" )
		{
			uint result = 0;
			if( func->function->GetReturnTypeId() == asTYPEID_BOOL )
			{
				for( uint index = 0, end = eventsbool.size( ); index < end; index++ )
					if( eventsbool[index]->function == func->function && eventsbool[index]->object == func->object)
					{
			...
			else if( func->function->GetReturnTypeId() == asTYPEID_VOID )
			{
				for( uint index = 0, end = eventsresult.size( ); index < end; index++ )
					if( eventsresult[index]->function == func->function && eventsresult[index]->object == func->object )
					{
		...
		else
		{
			...
			auto iterbrunch = brunchs.find( section );
			if( iterbrunch != brunchs.end( ) )
				return iterbrunch->second->EraseCallback( script_eventName, func, currentName );
		}
		return 0;
	}
} EventRoot;

void AddCallback( ScriptString& script_eventName, asIScriptFunction* func )
{
	string name = script_eventName.c_std_str();
	std::transform(name.begin(), name.end(), name.begin(), ::tolower);

	EventRoot.AddCallback( script_eventName, new EventFunction( func ), name );
}

uint EraseCallback( ScriptString& script_eventName, asIScriptFunction* func )
{
	string name = script_eventName.c_std_str();
	std::transform(name.begin(), name.end(), name.begin(), ::tolower);

	return EventRoot.EraseCallback( script_eventName, new EventFunction( func ), name );
}

uint EraseDelegate( ScriptString& script_eventName, ScriptString& script_methodname, void* object, int typeId )
{
	...
	if( type )
	{
		...
		if( func )
		{
			string name = script_eventName.c_std_str();
			std::transform(name.begin(), name.end(), name.begin(), ::tolower);
			result += EventRoot.EraseCallback( script_eventName, &EventFunction( func, object, typeId ), name );
		}
...
bool AddDelegate( ScriptString& script_eventName, ScriptString& script_methodname, void* object, int typeId )
{
	if( typeId & asTYPEID_OBJHANDLE )
	{
		// Store the actual reference
	...
	if( type )
	{
		...
		if( func )
		{
			string name = script_eventName.c_std_str();
			std::transform(name.begin(), name.end(), name.begin(), ::tolower);
			EventRoot.AddCallback( script_eventName, new EventFunction( func, object, typeId ), name );
			result = true;
		}
...
int script_CompileFunction( ScriptString& moduleName, ScriptString& code )
{
	// asIScriptModule
	asIScriptModule* module = ASEngine->GetModule( moduleName.c_str( ) );
	if( module )
		return module->CompileFunction( "Custom", code.c_str( ), -1, asCOMP_ADD_TO_MODULE, NULL );
	return 1;
}

int script_IsFunction( ScriptString& moduleName, ScriptString& declaration )
{
	// asIScriptModule
	asIScriptModule* module = ASEngine->GetModule( moduleName.c_str( ) );
	if( module )
		return module->GetFunctionIdByDecl( declaration.c_str( ) );
	return 1;
}

int script_DeleteFunction( ScriptString& moduleName, ScriptString& declaration )
{
	// asIScriptModule
	asIScriptModule* module = ASEngine->GetModule( moduleName.c_str( ) );
	if( module )
	{
		int id = module->GetFunctionIdByDecl( declaration.c_str( ) );
		if( id > 0 ) return module->RemoveFunction( id );
		return 2;
	}
	return 1;
}

bool RunEvent( ScriptString& script_eventName, ScriptHandle* handle )
{
	asIScriptContext* eventContext = GetEventContext( );
	string name = script_eventName.c_std_str();
	std::transform(name.begin(), name.end(), name.begin(), ::tolower);
	bool result = EventRoot.Run( script_eventName, handle, name, eventContext );
...
{	
	...
	// Trololo
	int r;
	r = ASEngine->RegisterObjectType( "ScriptType", sizeof( asIObjectType ), asOBJ_REF ); assert( r >= 0 );
	r = ASEngine->RegisterObjectBehaviour( "ScriptType", asBEHAVE_ADDREF, "void f()", asMETHOD( asIObjectType, AddRef ), asCALL_THISCALL ); assert( r >= 0 );
    r = ASEngine->RegisterObjectBehaviour( "ScriptType", asBEHAVE_RELEASE, "void f()", asMETHOD( asIObjectType, Release ), asCALL_THISCALL ); assert( r >= 0 );
	
	ASEngine->RegisterFuncdef("void EventFuncdefVoid( string&in event, Handle@+ handle )");
	ASEngine->RegisterFuncdef("bool EventFuncdefBool( string&in event, Handle@+ handle )");
	ASEngine->RegisterGlobalFunction( "void SetEventCallback( string& name, EventFuncdefVoid@+ value )", asFUNCTION( AddCallback ), asCALL_CDECL );
	ASEngine->RegisterGlobalFunction( "void SetEventCallback( string& name, EventFuncdefBool@+ value )", asFUNCTION( AddCallback ), asCALL_CDECL );
	ASEngine->RegisterGlobalFunction( "bool SetEventCallback( string& name, string& method, const ?&in )", asFUNCTION( AddDelegate ), asCALL_CDECL );
	ASEngine->RegisterGlobalFunction( "uint EraseEventCallback( string& name, EventFuncdefVoid@+ value )", asFUNCTION( EraseCallback ), asCALL_CDECL );
	ASEngine->RegisterGlobalFunction( "uint EraseEventCallback( string& name, EventFuncdefBool@+ value )", asFUNCTION( EraseCallback ), asCALL_CDECL );
	ASEngine->RegisterGlobalFunction( "uint EraseEventCallback( string& name, string& method, const ?&in )", asFUNCTION( EraseDelegate ), asCALL_CDECL );
    ASEngine->RegisterGlobalFunction( "bool RunEvent( string& , Handle@+ handle = null )", asFUNCTION( RunEvent ), asCALL_CDECL );
    ASEngine->RegisterGlobalFunction( "void ClearEventSystem( )", asFUNCTION( ClearEventSystem ), asCALL_CDECL );
    ASEngine->RegisterGlobalFunction( "void PrepareEventSystem( )", asFUNCTION( PrepareEventSystem ), asCALL_CDECL );
	
    ASEngine->RegisterGlobalFunction( "int CompileFunction( string&, string& )", asFUNCTION( script_CompileFunction ), asCALL_CDECL );
    ASEngine->RegisterGlobalFunction( "int IsFunction( string&, string& )", asFUNCTION( script_IsFunction ), asCALL_CDECL );
    ASEngine->RegisterGlobalFunction( "int DeleteFunction( string&, string& )", asFUNCTION( script_DeleteFunction ), asCALL_CDECL );
